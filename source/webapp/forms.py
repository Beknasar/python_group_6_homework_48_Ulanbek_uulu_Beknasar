...
class ProductForm(forms.Form):
    ...
    category = forms.ChoiceField(choices=CATEGORY_CHOICES, required=True, label='Категория', initial=DEFAULT_CATEGORY)
    amount = forms.IntegerField(min_value=0, required=True, label='Остаток')
    price = forms.DecimalField(min_value=0, required=True, label='Цена', max_digits=7, decimal_places=2)
